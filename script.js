...
function initScrollReveals() {
  const allTypingTargets = document.querySelectorAll(
    '.chapter-badge, .hero-badge, .hero-title, .hero-description, .section-label, .story-line, .photo-caption-tag, .memory-quote, .quote-highlight-box p, .montage-quote-line, .montage-caption, .flower-closing-message h2, .flower-closing-message p, .her-story-content h2, .memory-title, .expression-text-box h3, .us-featured-text-container h2, .montage-header h2'
  );
  ...
  const photoCards = document.querySelectorAll('.glass-photo-frame, .her-photo-card, .us-featured-photo-card');
    ...
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          const caption = entry.target.querySelector('.photo-caption-tag, .memory-quote');
  ...
  const textGroups = document.querySelectorAll('.her-story-content, .expression-card, .expression-text-box, .memory-block-item, .us-featured-block');
...
function initMouseParallax() {
  const cards = document.querySelectorAll('.glass-photo-frame, .glass-card');
  cards.forEach((card) => {
    ...
    card.addEventListener('mouseleave', () => {
      ...
      card.style.transform = `perspective(1000px) rotate(${defaultTilt}) translateY(0)`;
    });
  });
}
